...
from seahub.api2.views import get_dir_file_recursively, get_dir_file_size_info_recursively, calib_dir_file_response
...
class DirView(APIView):
    ...
    def get(self, request, repo_id, format=None):
        ...
        if recursive == '1':
            ...
            # add by wrx
            calib_dict = get_dir_file_size_info_recursively(username, repo_id, parent_dir, [], dir_file_info_list)
            calib_dir_file_response(response_dict, calib_dict)
            # add end
        ...
        # add by wrx
        calib_dict = get_dir_file_size_info_recursively(username, repo_id, parent_dir, [])
        calib_dir_file_response(response_dict, calib_dict)
        # add end
        return Response(response_dict)
