...
# add by wrx
def get_dir_file_size_info_recursively(username, repo_id, path, all_dirs, origin_list=None):
    if not origin_list:
        origin_list = get_dir_file_recursively(username, repo_id, path, all_dirs)
    result_dict = {}
    for entry in origin_list:
        if entry["type"] == "dir":
            if entry["id"] not in result_dict:
                result_dict[entry["parent_dir"]+entry["name"]] = {
                    "size": 0,
                    "fileSize": 0,
                    "numTotalFiles": 0,
                    "numFiles": 0,
                    "numDirs": 0,
                    "path": entry["parent_dir"]+entry["name"]
                }
    for entry in origin_list:
        if entry["type"] == "dir":
            if entry["parent_dir"] in result_dict:
                result_dict[entry["parent_dir"]]["numDirs"] += 1
        if entry["type"] == "file":
            ...
            for key, value in result_dict.items():
                if key == parent_dir:
                    ...
                    value["numTotalFiles"] += 1
                    value["numFiles"] += 1
                elif parent_dir.startswith(key+"/"):
                    value["size"] += entry["size"]
                    value["numTotalFiles"] += 1
    return result_dict


def calib_dir_file_response(origin_json, calib_dict):
    for entry in origin_json["dirent_list"]:
        if entry["type"] == "dir" and entry["parent_dir"]+entry["name"] in calib_dict:
            entry.update(calib_dict[entry["parent_dir"]+entry["name"]])
    return
# add end
